SELECT DISTINCT F.idFactura AS NumApartado, FP.Saldo,FP.FechaVencimiento,(SELECT FPP.`Saldo`+SUM(PFPP.`PagoContado`)+SUM(PFPP.`PagoTarjeta`) FROM Factura AS FF,FacturasPendientes AS FPP,PagosFactPendientes AS PFPP WHERE
...
ORDER BY F.idFactura DESC LIMIT 1000;
